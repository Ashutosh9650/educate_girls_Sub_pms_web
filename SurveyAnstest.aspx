<%@ Page Title="" Language="C#" AutoEventWireup="true" CodeFile="SurveyAnstest.aspx.cs" Inherits="SurveyAnstest" %>
 ...
 <script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        function savedata() {
            ...
            var listData = [];
            // var data = {};
            $('#WebSurveyTable tr').each(function () {
                        ...
                        if (elem.name == "Numeric" || elem.name == "Text" || elem.name == "Date") {
                            ...
                            if (elem.type == "radio") {
                                for (var i = 0; i < $(this).children()[2].children.length; i++) {
                                    if ($(this).children()[2].children[i].type == "radio") {
                                        if ($(this).children()[2].children[i].checked == true)
                                            listData.push({ "QuestionId": $(this).children()[2].children[i].name, "QuestionValue": $(this).children()[2].children[i].value })
                                        // {alert("Radio_"+$(this).children()[2].children[i].name + "_" + $(this).children()[2].children[i].value)}
                                    }
                                }
                            }
                                ...
                                var checkedValue = '';
                                for (var i = 0; i < $(this).children()[2].children.length; i++) {
                                    if ($(this).children()[2].children[i].type == "checkbox") {
                                        if ($(this).children()[2].children[i].checked == true) {
                                            if (checkedValue == '')
                                                checkedValue = $(this).children()[2].children[i].value;
                                            else
                                                checkedValue += "," + $(this).children()[2].children[i].value;
                                ...
                                if (checkedValue != '')
                                    listData.push({ "QuestionId": $(this).children()[2].children[0].name, "QuestionValue": checkedValue })
        ...
        function getBlock() {
            try {
                if ($('#ddldistrict').val() != 0) {
                    $.ajax({
                        type: "POST",
                        url: "Survey_District_Level.aspx/getBlock",
                        ...
                        success: function (response) {
                            if (response != 0) {
                                var respo = JSON.parse(response.d);
                                var list = '<option value="0">Select Block</option>'
                                $.each(respo, function (i, n) {
                                    list += '<option value="' + n.BlockID + '">' + n.Block + '</option>'
                                })
                                $('#ddlblock').html(list)
                            }
                            // alert(response.d);
                        ...
                        <div class="row">
                            <div class="col-xl-3 col-lg-3 col-md-3 col-sm-12 divsm"  runat="server" Visible="false">
                                <div class="form-group">
                                    ...
                                    <asp:DropDownList ID="ddlstate" CssClass="form-control" runat="server" Visible="false" onchange="getDistrict()">
                                    </asp:DropDownList>
                                    <asp:Label ID="lblstste" CssClass="form-control" runat="server"></asp:Label>
