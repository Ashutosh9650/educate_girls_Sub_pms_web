...
public partial class SurveyAnspre : System.Web.UI.Page
{
    ...
    protected void ddlParticipate_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DataTable dt = objBLL1.Get_DataFor8Filter("rptQuestionvaldationNew", ddlForm.SelectedValue, ddlParticipate.SelectedValue);
        if (dt.Rows.Count > 0)
        {
    ...
    public void FillQuestion1(int FormID)
    {
        DataTable dt = new DataTable();
        //hdnconditions.Value = Condition.ToString();
        dt = Get_DataFor3Filter("USP_GetQuestionTraining", "0", FormID.ToString(), "0");
        Session["Ism"] = dt;
    ...
    public void FillQuestion(int FormID)
    {
        DataTable dt = new DataTable();
        //hdnconditions.Value = Condition.ToString();
        dt = Get_DataFor3Filter("USP_GetQuestionTraining2024", "0", FormID.ToString(), "0");
        Session["Ism"] = dt;
        //DataListQuestion.DataSource = dt;
        //DataListQuestion.DataBind();
        fillQuestions(dt);
    ...
    public static string Savedata(string data, string StateID, string DistrictID, string Blockid, string FormID, string FinalFlag, string Year, string Month)
    {
        ...
        try
        {
            ...
            string point = "";
            clsMain objBLL = new clsMain();
            DataTable dtcheck = objBLL.Get_DataFor8Filter("rptQuestionvaldationNew", FID, DistrictID);
            ...
            else
            {
                ...
                {
                    DataTable dt = UploadFormEvaluation(finaldt, FID, FormEvalGUID.Trim(), "0", DistrictID, "0", "0", FinalFlag, "0", "0");
    ...
    public static string getBlock(string DistrictID)
    {
        ...
        if (dtP.Rows.Count > 0)
        {
            ...
            if (dr.Length > 0)
            {
                ...
                dt = objBLL1.Get_DataFor8Filter("rptQuestionvaldationNew", FID, Convert.ToString(dr[0]["ParticiparticipateID"]));
                if (dt.Rows.Count > 0)
                {
        ...
        //dt = objBLL1.Get_DataFor8Filter("rptQuestionvaldation",  FID, DistrictID);
        //if (dt.Rows.Count>0)
        //{
