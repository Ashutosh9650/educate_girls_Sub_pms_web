...
function Set_Date_Cal_Future(txtids) {
    var date = new Date();
    var today = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    //if ($('[id$=' + txtids + ']').val() == "") {
    //    var dv = ("0" + date.getDate()).slice(-2) + "/" + ("0" + (date.getMonth() + 1)).slice(-2) + "/" + date.getFullYear();
...
function Set_Date_Cal_DOBDOJ(txtids) {
    var date = new Date();
    var today = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    //if ($('[id$=' + txtids + ']').val() == "") {
    //    var dv = ("0" + date.getDate()).slice(-2) + "/" + ("0" + (date.getMonth() + 1)).slice(-2) + "/" + date.getFullYear();
...
function ShowGRPDownload(CanvasID, CanvasParentDivID, dwnFilesNames) {
    ...
    dwnBtnVar = '<a id="Anch' + CanvasID + '" class="btn btn-grp-download pull-right" onclick="showCHDnowloads(' + idvID + ')" style="top: 0px; position: absolute;right: 13px;background-color: #eee;"><i class="fa fa-download"></i></a>' +
        '<div class="DivChDwnBtn" id="pngdiv' + CanvasID + '" style="display: none;top: 25px; position: absolute;right: 13px;width: 66px;background-color: #eee; >' +
        /*'<a class="dropdown-item " href="javascript:void(0);" onclick="getPDF(' + dwPfnIDs + ')"><i class="fa fa-angle-double-right"></i>PDF</a>' +*/
        ' <a class="dropdown-item1" href="javascript:void(0);" onclick="download_image(' + dwPngIDs + ');"><i class="fa fa-angle-double-right"  style="margin-left:23px;"></i>PNG</a>' +
        ' <a class="dropdown-item1" href="javascript:void(0);" onclick="download_image(' + dwJpgIDs + ');"><i class="fa fa-angle-double-right"></i>JPG</a>' +
